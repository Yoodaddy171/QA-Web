    ...
    <Card variant="majestic">
      <CardHeader className="border-b border-border/40 pb-3">
        <div className="flex items-center justify-between gap-3">
          <CardTitle className="flex items-center gap-2 text-lg font-bold tracking-tight"><Bot className="h-5 w-5 text-primary" />Konfigurasi AI</CardTitle>
      ...
      <CardContent className="grid gap-4 pt-4 md:grid-cols-2">
        ...
        <div className="space-y-2"><Label>Groq API Key</Label><Input type={showKeys ? 'text' : 'password'} value={groqApiKey} onChange={event => setGroqApiKey(event.target.value)} placeholder="Masukkan Groq API key" autoComplete="off" /><p className="text-xs text-muted-foreground">Status: {formatStatus(groqStatus)}</p></div>
        <div className="space-y-2"><Label>Gemini API Key</Label><Input type={showKeys ? 'text' : 'password'} value={geminiApiKey} onChange={event => setGeminiApiKey(event.target.value)} placeholder="Masukkan Gemini API key" autoComplete="off" /><p className="text-xs text-muted-foreground">Status: {formatStatus(geminiStatus)}</p></div>
