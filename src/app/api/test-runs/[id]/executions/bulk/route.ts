...
export async function POST(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const result = await bulkExecuteTestCases({ projectId, testRunId: id, testCaseIds, status, tester: typeof body.tester === 'string' ? body.tester.trim() || null : null, notes: typeof body.notes === 'string' ? body.notes.trim() || null : null });
