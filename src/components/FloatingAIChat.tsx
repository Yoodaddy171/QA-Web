...
export function FloatingAIChat({
  ...
  const askAI = async (questionOverride?: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || 'AI chat gagal diproses.');
      }
  ...
  if (!open) {
    return (
      <button
        ...
        className="fixed bottom-6 right-6 z-50 flex h-14 w-14 items-center justify-center rounded-2xl bg-primary text-primary-foreground elevation-3 transition hover:-translate-y-0.5 hover:elevation-4 focus:outline-none focus:ring-4 focus:ring-primary/30"
